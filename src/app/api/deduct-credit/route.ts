...
export async function POST(request: NextRequest) {
  try {
    const { userId, songId, reason = 'Credit deduction' } = await request.json()
    ...
    const { data, error } = await adminClient
      .rpc('update_user_credits', {
        ...
        credit_change: -1, // Deduct 1 credit
        operation_type: 'song_creation',
    ...
    if (error) {
      logSecureError('Credit deduction failed', error, { userId, songId })
      return NextResponse.json(
    ...
    logSecureError('Credit deduction API error', error)
    return NextResponse.json(
      { error: sanitizeError(error, 'Failed to deduct credit') },
      { status: 500 }
    )
  }
}